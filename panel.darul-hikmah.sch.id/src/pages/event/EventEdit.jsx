...
import {update as updateEvent, show as showEvent} from "../../utils/api/event.jsx"
...
const EventCreate = () => {
    ...
    const OnSubmit = (e) => {
        e.preventDefault();
        setLoading(true);
        updateEvent(params).then (() => setLoading(false)).catch(() => setLoading(false));
    }
    ...
    useEffect(() => {
        showEvent(id).then((result) => {
            ...
            setEnd(new Date(result.end))
            setClipboard(result.content);
            setImage(result.image);
            setPlaceholder(result.placeholder);
            getGalleries({type: 'select'}).then((resp) => {
                setGallery(resp);
                const gal = resp.filter((item) => item.value === result.gallery_id).pop();
        ...
        <React.Fragment>
            ...
            <Content>
                <BlockHead size="lg">
                    ...
                    <BlockBetween>
                        <BlockHeadContent>
                            <BlockTitle className="page-title">UBAH Event</BlockTitle>
                            <BlockDes>
                    ...
                    <Row className={"gy-3"}>
                        <Col size={"8"}>
                            <PreviewCard>
                                ...
                                <div className="form-group">
                                    <div className="form-control-wrap">
                                        <QuillComponent clipboard={clipboard} setContent={setContent}/>
                                    </div>
                                </div>
                            </PreviewCard>
                            <PreviewCard>
