...
class APICore {
    ...
    handleError(error) {
        ...
        const messages = response ? response.data : error.message
        let message = Array.isArray(messages) ? messages[0].message : messages
        if (error.status) {
            switch (status) {
                case 403:
                    message = 'Anda tidak berhak mengakses layanan ini ini';
                    break;
                case 404:
